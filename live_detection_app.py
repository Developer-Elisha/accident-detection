...
def detect_accidents(video_source):
    ...
    accident_placeholder = st.empty()  # placeholder for accident notifications

    MIN_OVERLAP_AREA = 500  # Ignore tiny overlaps (e.g., shadows)
    ...
    while True:
        ...
        # collect boxes (person=0, car=2, motorbike=3, bus=5, truck=7)
        boxes = []
        ...
        for i in range(len(tracks)):
            for j in range(i + 1, len(tracks)):
                ...
                # register accident only if overlap is significant
...
if mode == "Upload Video":
    ...
    if st.button("Start Live Camera"):
        detect_accidents(0)  # 0 = default webcam
